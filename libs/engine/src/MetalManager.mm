...
auto metaldb::MetalManager::Create() noexcept -> std::shared_ptr<MetalManager> {
  ...
  std::cout << "Got Device: " << manager.device.description.UTF8String
            << std::endl;
  ...
  manager.pipeline =
      MetalManager::GetComputePipeline(manager.library, manager.constants);
  if (!manager.pipeline) {
    return nullptr;
  }
...
void metaldb::MetalManager::runCPU(
    const std::vector<char> &serializedData,
    const std::vector<metaldb::InstSerializedValue> &instructions,
    OutputBufferType &outputBuffer, size_t numRows) noexcept {
  // Mimic the GPU on the CPU
  // Force const cast the pointers (bad)
  const auto numInstructions = instructions.at(0);
  std::array<metaldb::OutputRow::NumBytesType,
             metaldb::DbConstants::MAX_NUM_ROWS>
      scratch;
  ...
  for (std::size_t thread = 0; thread < numRows; ++thread) {
    ...
    metaldb::RunInstructions(
        (metaldb::InstSerializedValue *)&instructions.at(1), numInstructions,
        constants);
  }
}

void metaldb::MetalManager::run(
    const std::vector<char> &serializedData,
    const std::vector<metaldb::InstSerializedValue> &instructions,
    OutputBufferType &outputBuffer, size_t numRows) noexcept {
  if (numRows == 0) {
  ...
  //        return this->runCPU(serializedData, instructions, outputBuffer,
  //        numRows);

  // Input buffer
  auto inputBuffer = [this->device
      newBufferWithLength:serializedData.size() * sizeof(serializedData.at(0))
                  options:MTLResourceStorageModeShared];
  auto instructionsBuffer = [this->device
      newBufferWithLength:(instructions.size() * sizeof(instructions.at(0))) + 1
                  options:MTLResourceStorageModeShared];
  auto outputBufferMtl =
      [this->device newBufferWithLength:outputBuffer.size()
                                options:MTLResourceStorageModeShared];
  std::cout << "Encoding Input Buffer with length: " << inputBuffer.length
            << std::endl;
  std::cout << "Encoding Instruction Buffer with length: "
            << instructionsBuffer.length << std::endl;
  std::cout << "Output buffer with length: " << outputBufferMtl.length
            << std::endl;

  for (std::size_t i = 0; i < serializedData.size(); ++i) {
    ((metaldb::InstSerializedValue *)inputBuffer.contents)[i] =
        serializedData.at(i);
  }

  for (std::size_t i = 0; i < instructions.size(); ++i) {
    ((OutputBufferType::value_type *)instructionsBuffer.contents)[i] =
        instructions.at(i);
  }
  ...
  //            error.debugDescription.UTF8String << std::endl;
  //        }
  ...
  std::cout << "Executing with Grid Size (" << gridSize.width << ","
            << gridSize.height << "," << gridSize.depth << ")" << std::endl;

  {
    ...
    [computeEncoder dispatchThreadgroups:MTLSizeMake(1, 1, 1)
                   threadsPerThreadgroup:gridSize];
...
auto metaldb::MetalManager::GetDevice() noexcept -> id<MTLDevice> _Nullable {
  auto check = [](id<MTLDevice> device) { return !device.isLowPower; };
...
auto metaldb::MetalManager::GetLibrary(id<MTLDevice> _Nonnull device) noexcept
    -> id<MTLLibrary> _Nullable {
  NSError *error = nullptr;
  auto library = [device
      newLibraryWithFile:@"../libs/engine_shaders/MetalDbEngine.metallib"
                   error:&error];

  if (error) {
    std::cerr << "Failed while retrieving library: "
              << error.debugDescription.UTF8String << std::endl;
    return nullptr;
  }
...
auto metaldb::MetalManager::GetFunction(
    NSString *_Nonnull funcName, id<MTLLibrary> _Nonnull library,
    MTLFunctionConstantValues *_Nonnull constants) noexcept
    -> id<MTLFunction> _Nullable {
  NSError *error = nullptr;
  auto function = [library newFunctionWithName:funcName
                                constantValues:constants
                                         error:&error];
  if (error) {
    std::cerr << "Failed while retrieving function: "
              << error.debugDescription.UTF8String << std::endl;
    return nullptr;
  }
...
auto metaldb::MetalManager::GetEntryFunction(
...
auto metaldb::MetalManager::GetInternalBinaryFunction(
    NSString *_Nonnull funcName, id<MTLLibrary> _Nonnull library) noexcept
    -> id<MTLFunction> _Nullable {
  ...
  NSError *error = nullptr;
  auto function = [library newFunctionWithDescriptor:functionDescriptor
                                               error:&error];
  if (error) {
    std::cerr << "Failed to compile function: " << funcName.UTF8String
              << " got error: " << error.debugDescription.UTF8String
              << std::endl;
    return nullptr;
  }
...
auto metaldb::MetalManager::GetComputePipeline(
    id<MTLLibrary> _Nonnull library,
    MTLFunctionConstantValues *_Nonnull constants) noexcept
    -> id<MTLComputePipelineState> _Nullable {
  ...
  // A pipeline runs a single function, optionally manipulating the input data
  // before running the function, and the output data afterwards. Creating the
  // pipeline finishes compiling the shader for the GPU.
  NSError *error = nullptr;
  auto pipeline = [library.device
      newComputePipelineStateWithDescriptor:computeDescriptor
                                    options:MTLPipelineOptionNone
                                 reflection:nil
                                      error:&error];
  if (error) {
    std::cerr << "Failed while creating pipeline: "
  ...
  if (linkedFunctions.binaryFunctions.count > 0) {
    ...
    auto decodeInstructionTable =
        [pipeline newVisibleFunctionTableWithDescriptor:vft];
