...
auto metaldb::Scheduler::SerializeRawTable(
    const metaldb::reader::RawTable &rawTable,
    std::size_t maxChunkSize) noexcept
    -> std::vector<std::pair<IntermediateBufferTypePtr, std::size_t>> {
  ...
  auto prepareChunk = [&](NumRowsType startRow, NumRowsType endRow) {
    ...
    // Write the row indexes
    {
      ...
      for (auto row = startRow; row < endRow; ++row) {
        // Last data access stores the highest index on the last iteration,
        // which we offset because those rows are not in this batch.
    ...
    // Final cleanup
    WriteBytesStartingAt(&rawDataSerialized->at(RawTable::SizeOfDataOffset),
                         numBytes);
    WriteBytesStartingAt(&rawDataSerialized->at(RawTable::SizeOfHeaderOffset),
                         sizeOfHeader);
...
auto metaldb::Scheduler::schedule(const QueryEngine::QueryPlan &plan) noexcept
    -> tf::Taskflow {
...
auto metaldb::Scheduler::MakeOutputBufferPtr() noexcept
    -> std::shared_ptr<MetalManager::OutputBufferType> {
  return std::make_shared<MetalManager::OutputBufferType>();
}

auto metaldb::Scheduler::registerStage(
    tf::Task &taskDoWork, const std::shared_ptr<QueryEngine::Stage> &stage,
    tf::Taskflow *_Nonnull taskflow, std::shared_ptr<MetalManager> manager,
    IntermediateBufferTypePtr outputBuffer) noexcept -> tf::Task {
  ...
  for (auto &child : stage->children) {
    ...
    Scheduler::registerStage(childDoWork, child, taskflow, manager,
                             childBuffer);
    childDoWork.precede(taskDoWork);
    childOutputBuffers.push_back(childBuffer);
  }

  Scheduler::registerBaseStage(taskDoWork, stage, taskflow, manager,
                               std::move(childOutputBuffers), outputBuffer);
  return taskDoWork;
}

void metaldb::Scheduler::registerBaseStage(
    tf::Task &taskDoWork, const std::shared_ptr<QueryEngine::Stage> &stage,
    tf::Taskflow *_Nonnull taskflow, std::shared_ptr<MetalManager> manager,
    std::vector<IntermediateBufferTypePtr> &&childOutputBuffers,
  ...
  auto encodeWorkTask = [&] {
    Parameters parameters{taskflow,    encoder,        &taskDoWork,
                          manager,     serializedData, childOutputBuffers,
                          outputBuffer};
  ...
  taskDoWork
      .work([=](tf::Subflow &subflow) {
        // Doing GPU work.
        std::vector<decltype(MakeOutputBufferPtr())> subtaskOutputBuffers;
        auto mergeSubtasks = subflow.placeholder();

        // Copy data from all child buffers into input, split by threadgroup
        // size. Split into new subflows for each 'group', that can all run in
        // parallel.
        ...
        auto submitWork = [&] {
          ...
          subflow
              .emplace([=]() {
                manager->run(*localCurrentInputBuffer, encoder->data(),
                             *subtaskNewBuffer, numRows);
              })
              .name("Do Work Chunk")
        ...
        for (auto &childBufferPtr : childOutputBuffers) {
          if (!childBufferPtr) {
            std::cout << "Child buffer pointer not initialized, skipping"
                      << std::endl;
            continue;
          }
...
auto metaldb::Scheduler::registerBasePartial(
    const std::shared_ptr<QueryEngine::StagePartial> &partial,
    Parameters &parameters) noexcept -> tf::Task {
  // Each partial returns the task to encode the instructions.
  tf::Task task;
  if (auto read =
          std::dynamic_pointer_cast<QueryEngine::ReadPartial>(partial)) {
    task = Scheduler::registerReadPartial(read, parameters);

  } else if (auto projection =
                 std::dynamic_pointer_cast<QueryEngine::ProjectionPartial>(
                     partial)) {
    task = Scheduler::registerProjectionPartial(projection, parameters);

  } else if (auto write = std::dynamic_pointer_cast<QueryEngine::WritePartial>(
                 partial)) {
    task = Scheduler::registerWritePartial(write, parameters);

  } else if (auto output =
                 std::dynamic_pointer_cast<QueryEngine::ShuffleOutputPartial>(
                     partial)) {
...
auto metaldb::Scheduler::registerReadPartial(
    std::shared_ptr<QueryEngine::ReadPartial> read,
    Parameters &parameters) noexcept -> tf::Task {
  std::cout << "Registering Read partial" << read->id() << " " << read->filepath
            << std::endl;
  ...
  auto readRawTableTask =
      parameters.taskflow
          ->emplace([=]() {
            std::cout << "Running read command for file: " << filename
                      << std::endl;
            std::filesystem::path path;
            path.append(filename);
            reader::CSVReader reader(path);
            std::cout << "Table Is Valid: " << (reader.IsValid() ? "YES" : "NO")
                      << std::endl;
  ...
  // This reads in a file (1 chunk) and splits it into serialized sub-chunks
  // each with a max row count of `maxNumRows` (metal/implementation defined).
  // The GPU is guaranteed to always return `OutputRow` buffers, so we can merge
  // them together.
  ...
  parameters.doWorkTask
      ->work([=](tf::Subflow &subflow) mutable {
        // Chunk the work out.
        std::vector<decltype(MakeOutputBufferPtr())> subtaskOutputBuffers;
        auto mergeSubtasks = subflow.placeholder();
        ...
        auto subtaskBuffers = [&]() {
          ...
          while (true) {
            auto subtaskBuffers =
                Scheduler::SerializeRawTable(*rawTablePtr, currentMaxNumRows);
            // Verify none exceed the max length
            bool allPassed = true;
            for (const auto &[childBufferPtr, numRows] : subtaskBuffers) {
              if (childBufferPtr->size() >= maxNumBytes) {
                // Re-serialize
                const double sizeOfRowApprox =
                    childBufferPtr->size() / (double)numRows;
                std::size_t approxMaxNumRows =
                    std::floor(maxNumBytes / sizeOfRowApprox);
                // Never make it larger
                currentMaxNumRows =
                    std::min(currentMaxNumRows - 1, approxMaxNumRows);
                allPassed = false;
                break;
              }
            }
        ...
        for (auto &[childBufferPtr, numRows] : subtaskBuffers) {
          ...
          subflow
              .emplace([=]() {
                ...
                manager->run(*bufferPtr, encoder->data(), *localOutput,
                             numRowsLocal);
              })
              .name("Do Work Chunk")
              .precede(mergeSubtasks);
        ...
        mergeSubtasks
            .work([=]() mutable {
              ...
              std::cout << "Writing output -- Num Columns: "
                        << (int)writer.NumColumns()
                        << " -- Num Bytes: " << (int)writer.NumBytes()
                        << " -- Num Rows: " << (int)writer.CurrentNumRows()
                        << std::endl;
              auto reader = OutputRowReader(*outputBuffer);
              std::cout << "Reading -- Num Columns: "
                        << (int)reader.NumColumns()
                        << " -- Num Bytes: " << (int)reader.NumBytes()
                        << " -- Num Rows: " << (int)reader.NumRows()
                        << std::endl;
            })
            .name("Merge subtasks");
      })
  ...
  return parameters.taskflow
      ->emplace([=]() {
        // Encode the commands
        auto columnTypes = [&] {
          std::vector<ColumnType> columnTypes;
          std::transform(definition->columns.begin(), definition->columns.end(),
                         std::back_inserter(columnTypes), [](auto col) {
...
auto metaldb::Scheduler::registerProjectionPartial(
    std::shared_ptr<QueryEngine::ProjectionPartial> projection,
    Parameters &parameters) noexcept -> tf::Task {
  std::cout << "Registering Projection partial" << projection->id()
            << std::endl;
...
auto metaldb::Scheduler::registerShufflePartial(
    std::shared_ptr<QueryEngine::ShuffleOutputPartial> output,
    Parameters &parameters) noexcept -> tf::Task {
...
auto metaldb::Scheduler::registerWritePartial(
    std::shared_ptr<QueryEngine::WritePartial> write,
    Parameters &parameters) noexcept -> tf::Task {
  ...
  return parameters.taskflow
      ->emplace([=]() {
        // This can just happen after the 'doWorkTask' because it must be the
        // last thing.
      })
      .name("Write output");
}
