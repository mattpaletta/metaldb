...
namespace metaldb {
     ...
     */
    template <typename Val, typename T> static Val ReadBytesStartingAt(T METAL_DEVICE* ptr) CPP_NOEXCEPT {
     ...
     * The caller should ensure that the bytes to read are in fact of type `Val` and the pointer is not null.
     */
    template <typename Val, typename T> static Val ReadBytesStartingAt(T METAL_THREAD* ptr) CPP_NOEXCEPT {
        if CPP_CONSTEXPR (sizeof(Val) == sizeof(T)) {
            return (Val)*ptr;
        } else {
     ...
     */
    template <typename Val, typename T> static void WriteBytesStartingAt(T METAL_DEVICE* ptr, const Val METAL_THREAD& val) CPP_NOEXCEPT {
     ...
     * Where N is the `sizeof(Val)`
     */
    template <typename Val, typename T> static void WriteBytesStartingAt(T METAL_THREAD* ptr, const Val METAL_THREAD& val) CPP_NOEXCEPT {
        if CPP_CONSTEXPR (sizeof(T) == sizeof(Val)) {
            *ptr = val;
        } else {
            for (size_t n = 0; n < (sizeof(Val) / sizeof(T)); ++n) {
     ...
     */
    template <typename Val, typename T> static void WriteBytesStartingAt(std::vector<T>& ptr, const Val& val) CPP_NOEXCEPT {
        union {
            Val a;
            T bytes[sizeof(Val)];
        } thing;
